...
def get_data():
    ...
    :return:
    """
    with open("data/ham_data.txt", encoding="utf8") as ham, open("data/spam_data.txt", encoding="utf8") as spam:
        ham_data = ham.readlines()
        spam_data = spam.readlines()
...
def main():
    ...
    # 数据归整化和预处理
    norm_train_corpus = normalization.normalize_corpus(train_corpus)
    norm_test_corpus = normalization.normalize_corpus(test_corpus)

    # 词袋模型特征
    bow_vectorizer, bow_train_features = fe.bow_extractor(norm_train_corpus)
    bow_test_features = bow_vectorizer.transform(norm_test_corpus)

    # TF-IDF 特征
    tfidf_vectorizer, tfidf_train_features = fe.tfidf_extractor(norm_train_corpus)
